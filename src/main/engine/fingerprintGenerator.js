/**
 * fingerprintGenerator.js — Random fingerprint generation for browser profiles.
 *
 * Generates realistic, internally-consistent browser fingerprints.
 * Each generated fingerprint ensures that all properties are coherent:
 *   - User-Agent matches OS, browser, and version
 *   - Platform string matches OS
 *   - WebGL vendor/renderer matches typical hardware for that OS
 *   - Screen resolution matches common monitor sizes
 *   - Language/timezone are geographically plausible
 *
 * Uses a seeded PRNG so the same seed always produces the same fingerprint,
 * enabling reproducibility while still appearing unique.
 */
